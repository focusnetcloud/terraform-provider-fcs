...
page_title: "fcs_business_cluster Resource - FCS"
subcategory: ""
description: |-
  An FCS Business cluster (dedicated virtual cluster) inside an environment. CIDRs, IPs and VLANs are allocated server-side. Create is asynchronous: the provider polls until status=active. Destroy polls until the cluster is gone so the environment is only torn down afterwards. Import uses <environment_id>/<cluster_id>. Sizing is read back from the API and changes are applied in place; the cluster ID and Kubernetes API remain unchanged.
---

# fcs_business_cluster (Resource)
...
- `create` (String) How long to wait for the cluster to reach status=active (default 20m0s). This is an object attribute: configure it as timeouts = { create = "30m" }; a timeouts { ... } block is invalid.
...
- `update` (String) How long to wait for an in-place resize to reach the requested size (default 20m0s). Configure it inside the same timeouts = { ... } object.
