...
subcategory: ""
description: |-
  An FCS Business cluster (dedicated virtual cluster) inside an environment. CIDRs, IPs and VLANs are allocated server-side. Create is asynchronous: the provider polls until status=active. Destroy polls until the cluster is gone so the environment is only torn down afterwards.
---
...
- `create` (String) How long to wait for the cluster to reach status=active (default 20m0s). Accepts a duration string such as "30m".
